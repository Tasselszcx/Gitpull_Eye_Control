...
# 串口读取超时（秒）：避免 busy-wait 抢占 CPU/GIL
SERIAL_TIMEOUT = 0.1

# 3. 想要在屏幕上显示多少个点 (调整窗口宽度)
MAX_POINTS = 200  

# 实时绘图刷新间隔（毫秒）：太小会占用大量CPU，影响串口读取
PLOT_INTERVAL_MS = 50
...
def read_serial_data():
    ...
    while is_running:
        try:
            # 带超时的阻塞读取：减少 busy-wait，避免绘图线程抢占导致串口丢数据
            raw = ser.readline()
            if not raw:
                continue
            ...
            if line and ":" in line:
                ...
                if len(parts) >= 2:
                    # 解析第一列
                    label1, val1 = parts[0].split(':', 1)
                    # 解析第二列
                    label2, val2 = parts[1].split(':', 1)
                    ...
                    if labels[0] == "Channel 1":
                        labels[0] = label1.strip()
                        labels[1] = label2.strip()
                        print(f"检测到数据源: {labels[0]} 和 {labels[1]}")
...
# ---- 绘图动画函数 ----
def animate(i):
    global _last_plot_labels
    y1 = list(data_ch1)
    y2 = list(data_ch2)
    line1.set_ydata(y1)
    line2.set_ydata(y2)

    # 标签变化时才更新标题/图例，避免每帧重建
    cur_labels = (labels[0], labels[1])
    if cur_labels != _last_plot_labels:
        line1.set_label(labels[0])
        line2.set_label(labels[1])
        ax.set_title(f'Real-time Sensor Data: {labels[0]} & {labels[1]}')
        ax.legend(loc='upper right')
        _last_plot_labels = cur_labels
...
def init_plot():
    global line1, line2, _last_plot_labels
    ...
    line1, = ax.plot(x, list(data_ch1), label=labels[0], color='blue', linewidth=1.5)
    line2, = ax.plot(x, list(data_ch2), label=labels[1], color='red', linewidth=1.5, alpha=0.8)
    ax.legend(loc='upper right')
    _last_plot_labels = (labels[0], labels[1])
    return line1, line2
