/*
 * [Mapping Update] EOG AI + IMU Mouse
 * 映射更改：Left -> 鼠标左键 | Blink -> 无动作
 */
...
void setup() {
    ...
    Serial.println("🚀 EOG Mouse: Left-Click Mode Ready!");
}

void loop() {
    // --- Step A: IMU 头部控制移动 ---
    int16_t gx = 0, gy = 0;  // 提到 loop 开头，供 Step D 串口输出使用
    Wire.beginTransmission(IMU_ADDR);
    Wire.write(0x43);
    ...
    if (Wire.available() >= 6) {
        ...
        if (gx != 0 || gy != 0) Mouse.move(gx * SENSITIVITY, gy * SENSITIVITY);
    }
    ...
    if (lock_counter > 0) {
        ...
        if (raw_features[1] > 60.0 || raw_features[8] > 60.0) {
            ...
            if (pred_idx != 0) {
                ...
                // --- 🌟 动作执行映射 (修改处) ---
                if (final_idx == 3) {        // 向左看
                    Mouse.click(MOUSE_LEFT); // 触发左键单击
                } 
                else if (final_idx == 4) {   // 向右看
                    Mouse.click(MOUSE_RIGHT); // 触发右键单击
                }
